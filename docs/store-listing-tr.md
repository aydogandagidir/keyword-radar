...
Türkçe CWS listesi için kopyala-yapıştır kaynağı. Alan limitlerine dikkat edin.

## Başlık (en fazla 45 karakter)

```
Keyword Radar — Pazaryeri Kelime Aracı
```
...
• Yalnızca-görünür toplama — canlı otomatik tamamlama listesini okur; gizli uç nokta yok, özel veri kazıma yok.
• Türkçe-duyarlı normalizasyon — ç ğ ı ö ş ü içeren varyantlar bölünmek yerine doğru birleşir.
• Fırsat skoru — frekans, uzun-kuyruk, pazaryeri kapsamı ve güven tek bir 0–100 skorunda.
• Listing gap analizi — toplanan kelimeleri ürün başlığınız ve açıklamanızla karşılaştırın.
• Pazaryeri kapsamı — hangi kelimenin hangi pazaryerinde göründüğünü görün.
• Dışa aktarma — tek tıkla CSV (Excel uyumlu) ve çok-sayfalı XLSX (özet, kelime, analiz, gap).
• Kayıtlı çalışmalar — toplama oturumlarını pazaryeri bazında yerel tutun.
...
5. Kelimeler / Kapsam / Aksiyonlar / Listing Gap sekmelerini inceleyin.
6. Kopyalayın, CSV/XLSX dışa aktarın veya çalışmayı kaydedin.
...
SINIRLAMALAR
• Pazaryerleri zaman zaman sayfa yapısını değiştirir; adaptörler yedek seçiciler kullanır ve güvenli biçimde başarısız olur (boş liste, asla çökme).
• Otomatik tamamlama önerileri bir talep sinyalidir, garantili arama hacmi sayısı değildir.
