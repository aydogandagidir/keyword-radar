...
Copy-paste source for the English CWS listing. Keep within the field limits noted.

## Title (max 45 chars)

```
Keyword Radar — Marketplace Keyword Tool
```
...
• Visible-only collection — reads the live autocomplete dropdown; no hidden endpoints, no scraping of private data.
• Turkish-aware normalization — variants with ç ğ ı ö ş ü collapse correctly instead of fragmenting your data.
• Opportunity scoring — frequency, long-tail, marketplace coverage and confidence combined into a single 0–100 score.
• Listing gap analysis — compare collected keywords against your product title and description.
• Marketplace coverage — see which keywords appear on which marketplaces.
• Export — one-click CSV (Excel-friendly) and multi-sheet XLSX (summary, keywords, analysis, gaps).
• Saved runs — keep collection sessions locally, per marketplace.
...
5. Explore the Words / Coverage / Actions / Listing Gap tabs.
6. Copy, export CSV/XLSX, or Save the run.
...
LIMITATIONS
• Marketplaces change their page markup over time; the adapters use fallback selectors and fail safely (an empty list, never a crash).
• Autocomplete suggestions are a demand signal, not guaranteed search-volume numbers.
...
- `Keyword Radar — TR Marketplace Research` (39)
- `Bluedev Keyword Radar` (21)
